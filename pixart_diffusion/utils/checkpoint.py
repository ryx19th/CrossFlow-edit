...
def load_checkpoint(checkpoint,
    ...
    null_embed = torch.load(f'pixart_pretrained_models/null_embed_diffusers_{max_length}token.pth', map_location='cpu')
    ...
    if state_dict["final_layer.linear.weight"].shape != model.final_layer.linear.weight.shape[0]:
        ...
        state_dict["final_layer.linear.bias"] = state_dict["final_layer.linear.bias"][:model.final_layer.linear.bias.shape[0]]
