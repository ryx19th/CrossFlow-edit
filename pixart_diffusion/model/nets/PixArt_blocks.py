...
class CaptionEmbedder(nn.Module):
    ...
    def forward(self, caption, train, force_drop_ids=None):
        # st() # NOTE Although during training the dropped caption will be replaced with y_emb, during inference the null caption is still "", not this y_emb????? Why? GAP? NOTE 
        # if train: # TODO Why only assert when training? Inference how different? TODO 
        if caption.shape[2:] != self.y_embedding.shape:
            assert caption.shape[2] == self.y_embedding.shape[0] * 2
            self.y_embedding = torch.cat([self.y_embedding, self.y_embedding], dim=0) # only run once 
