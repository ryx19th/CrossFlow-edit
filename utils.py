...
def initialize_train_state(config, device, nnet=None):

    nnet = get_nnet(**config.nnet) if nnet is None else nnet
    nnet_ema = get_nnet(**config.nnet) if nnet is None else copy.deepcopy(nnet)
    nnet_ema.eval()
